...
fn capture(root: &std::path::Path, id: &str, url: &str) -> serde_json::Value {
    ...
    archive::put_capture(root, json!({"id":uuid::Uuid::new_v4().to_string(),
        "postId":id,"archive":value})).unwrap();
...
fn shares_completed_url_across_posts_and_retries_commit() {
    ...
    assert_eq!(archive::append(root.path(), &job.id, lease, 0, bytes).unwrap(), bytes.len() as u64);
    assert_eq!(archive::append(root.path(), &job.id, lease, 0, bytes).unwrap(), bytes.len() as u64);
    let checksum = format!("{:x}", Sha256::digest(bytes));
    let receipt = archive::commit(root.path(), &job.id, lease, bytes.len() as u64, &checksum).unwrap();
    assert_eq!(receipt.name, format!("url_sha256_{}.png", archive::hash_url(url).unwrap()));
    assert_eq!(archive::commit(root.path(), &job.id, lease, bytes.len() as u64, &checksum).unwrap().name, receipt.name);
    assert!(archive::pull(root.path()).unwrap().is_none());
    assert_eq!(archive::ensure_resource(root.path(), "456", &archive::hash_url(url).unwrap()).unwrap().state, "stored");
}
...
fn rejects_unowned_resources_and_conflicting_revisions() {
    ...
    assert!(archive::ensure_resource(root.path(), "123", &archive::hash_url("https://example.com/other.png").unwrap()).is_err());
...
fn invalid_complete_media_stays_terminal_after_restart() {
    ...
    assert_eq!(archive::status(root.path(), &job.id).unwrap().state, "unsupported");
    assert!(archive::pull(root.path()).unwrap().is_none());
    assert!(!root.path().join(format!(".reflect/x-archive/transfers/{}.part", job.id)).exists());
}
