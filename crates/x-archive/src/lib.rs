...
pub fn valid_id(id: &str) -> bool {
    !id.starts_with('0') && !id.is_empty() && id.len() <= 20 && id.bytes().all(|b| b.is_ascii_digit())
}
...
pub fn get_candidate_names(hash: &str) -> Result<Vec<String>> {
    ...
    Ok(MEDIA_EXTENSIONS.iter().map(|ext| format!("url_sha256_{hash}.{ext}")).collect())
}
pub fn safe_path(root: &Path, relative: &str) -> Result<PathBuf> {
    ensure!(root.is_absolute() && root.is_dir(), "invalid-root");
    let mut path = root.to_path_buf();
    for component in Path::new(relative).components() {
        let Component::Normal(name) = component else { bail!("invalid-path") };
        path.push(name);
...
pub fn read_json(root: &Path, relative: &str) -> Result<Option<Value>> {
    ...
    ensure!(file.metadata()?.len() <= MESSAGE_MAX_BYTES as u64, "payload-too-large");
...
fn with_state<T>(root: &Path, action: impl FnOnce(&mut State) -> Result<T>) -> Result<T> {
    ...
    let lock = OpenOptions::new().create(true).read(true).write(true).truncate(false).open(lock_path)?;
    lock.lock_exclusive()?;
    ...
    atomic_json(root, ".reflect/x-archive/state.json", &serde_json::to_value(&state)?)?;
    FileExt::unlock(&lock)?;
    result
}
fn sniff(path: &Path) -> Result<(String, String, u64)> {
    ...
    let extension = match kind.mime_type() {
        ...
        "image/webp" => "webp",
        "image/gif" => "gif",
        "video/mp4" => { ensure!(bytes <= VIDEO_MAX_BYTES, "video-too-large"); "mp4" },
...
pub fn find_cache(root: &Path, hash: &str) -> Result<Option<Receipt>> {
    for name in get_candidate_names(hash)? {
        ...
        let Ok((extension, mime, bytes)) = sniff(&path) else { continue };
        if !name.ends_with(&format!(".{extension}")) { continue; }
        // The first valid candidate wins silently.
        return Ok(Some(Receipt { name, bytes, mime, sha256: None }));
    }
    Ok(None)
}
...
fn add_jobs(state: &mut State, archive: &Value) -> Result<()> {
    ...
    for raw in archive["resources"].as_array().context("invalid-resources")? {
        ...
        let job = state.jobs.entry(id.clone()).or_insert_with(|| Job {
            id, resource: resource.clone(), post_ids: vec![], state: resource.state.clone(),
            lease: None, lease_until: 0, offset: 0, receipt: None,
        });
        if !job.post_ids.iter().any(|id| id == post_id) { job.post_ids.push(post_id.into()); }
...
pub fn put_capture(root: &Path, envelope: Value) -> Result<()> {
    let id = envelope["id"].as_str().context("invalid-event")?.to_string();
    Uuid::parse_str(&id)?;
    let archive = &envelope["archive"];
    let post_id = envelope["postId"].as_str().context("invalid-post")?;
    ensure!(valid_id(post_id) && archive["id"] == post_id && archive["data"]["id"] == post_id, "wrong-post");
    ensure!(serde_json::to_vec(&envelope)?.len() <= MESSAGE_MAX_BYTES, "payload-too-large");
    with_state(root, |state| {
        if state.captures.contains_key(&id) { return Ok(()); }
...
pub fn mark_processed(root: &Path, event: &str) -> Result<()> {
    ...
    with_state(root, |state| { state.processed.insert(event.into(), true); Ok(()) })
}
pub fn read_post(root: &Path, post_id: &str) -> Result<Option<Value>> {
    ensure!(valid_id(post_id), "invalid-post");
    let value = read_json(root, &format!("assets/x/post-{post_id}.json"))?;
    if let Some(value) = &value {
        ensure!(value["id"] == post_id && value["data"]["id"] == post_id, "wrong-post");
    }
    Ok(value)
}
pub fn write_post(root: &Path, post_id: &str, expected: Option<&str>, value: &Value) -> Result<bool> {
    ensure!(valid_id(post_id) && value["id"] == post_id && value["data"]["id"] == post_id, "wrong-post");
    ensure!(serde_json::to_vec(value)?.len() <= MESSAGE_MAX_BYTES, "payload-too-large");
    with_state(root, |_| {
        let previous = read_post(root, post_id)?;
        if previous.as_ref().and_then(|value| value["revision"].as_str()) != expected { return Ok(false); }
...
pub fn ensure_resource(root: &Path, post_id: &str, hash: &str) -> Result<Job> {
    ...
    let raw = archive["resources"].as_array().context("invalid-resources")?.iter().find(|raw| {
        raw["url"].as_str().is_some_and(|url| hash_url(url).ok().as_deref() == Some(hash))
    }).context("resource-not-owned")?.clone();
    let resource: Resource = serde_json::from_value(raw)?;
    with_state(root, |state| {
        let id = format!("url_sha256_{hash}");
        let job = state.jobs.entry(id.clone()).or_insert_with(|| Job {
            id, resource: resource.clone(), post_ids: vec![post_id.into()],
            state: resource.state.clone(), lease: None, lease_until: 0, offset: 0, receipt: None,
        });
        if !job.post_ids.iter().any(|id| id == post_id) { job.post_ids.push(post_id.into()); }
        if let Some(receipt) = find_cache(root, hash)? {
            job.state = "stored".into(); job.receipt = Some(receipt);
        } else if job.state == "stored" {
            job.state = "pending".into(); job.receipt = None;
        }
        Ok(job.clone())
    })
}
pub fn pull(root: &Path) -> Result<Option<Job>> {
    with_state(root, |state| {
        for job in state.jobs.values_mut() {
            ...
            if let Some(receipt) = find_cache(root, hash)? {
                job.state = "stored".into(); job.receipt = Some(receipt); continue;
            }
            if job.state == "stored" { job.state = "pending".into(); job.receipt = None; }
            if job.state == "failed" && job.resource.error.as_deref() == Some("network")
                && job.lease_until <= now() {
                job.state = "pending".into(); job.lease = None;
            }
            if job.state != "pending" && job.state != "downloading" { continue; }
            if job.lease.is_some() && job.lease_until > now() { continue; }
...
fn leased<'a>(state: &'a mut State, id: &str, lease: &str) -> Result<&'a mut Job> {
    ...
    ensure!(job.lease.as_deref() == Some(lease) && job.lease_until >= now(), "lease-expired");
    job.lease_until = now() + LEASE_MS;
    Ok(job)
}
pub fn status(root: &Path, id: &str) -> Result<Job> {
    with_state(root, |state| state.jobs.get(id).cloned().context("unknown-job"))
}
pub fn append(root: &Path, id: &str, lease: &str, offset: u64, bytes: &[u8]) -> Result<u64> {
    ensure!(!bytes.is_empty() && bytes.len() <= CHUNK_MAX_BYTES, "invalid-chunk");
    with_state(root, |state| {
        ...
        } else { prefix.extend_from_slice(&bytes[..bytes.len().min(8192)]); }
        let video = infer::get(&prefix).is_some_and(|kind| kind.mime_type() == "video/mp4");
        let limit = if video { VIDEO_MAX_BYTES } else { IMAGE_MAX_BYTES };
        let end = offset.checked_add(bytes.len() as u64).context("invalid-offset")?;
        if end > limit {
            job.state = "unsupported".into(); job.resource.error = Some(if video { "video-too-large" } else { "format" }.into());
            let path = part_path(root, id)?;
            if path.exists() { fs::remove_file(path)?; }
        ...
        if file.metadata()?.len() > job.offset { file.set_len(job.offset)?; }
...
pub fn commit(root: &Path, id: &str, lease: &str, length: u64, expected_hash: &str) -> Result<Receipt> {
    with_state(root, |state| {
        let job = state.jobs.get_mut(id).context("unknown-job")?;
        if job.state == "stored" {
        ...
        let job = leased(state, id, lease)?;
        ensure!(valid_hash(expected_hash) && job.offset == length, "invalid-checksum");
        let path = part_path(root, id)?;
        let (extension, mime, bytes) = match sniff(&path) {
            Ok(media) => media,
            Err(error) => {
                let reason = error.to_string();
                let format = ["unsupported-format", "invalid-file", "media-too-large", "video-too-large"]
                    .contains(&reason.as_str());
                job.state = if format { "unsupported" } else { "failed" }.into();
                job.resource.error = Some(if reason == "video-too-large" {
                    "video-too-large"
                } else if format { "format" } else { "storage" }.into());
                job.offset = 0;
                if path.exists() { fs::remove_file(&path)?; }
                return Err(error);
            }
        ...
        if let Some(receipt) = find_cache(root, hash)? {
            fs::remove_file(&path)?;
            job.receipt = Some(receipt.clone()); job.state = "stored".into(); return Ok(receipt);
        }
        directory(root, "assets/x")?;
        let name = format!("{id}.{extension}");
        let final_path = safe_path(root, &format!("assets/x/{name}"))?;
        fs::rename(&path, &final_path)?;
        #[cfg(unix)] File::open(final_path.parent().context("parent")?)?.sync_all()?;
        let receipt = Receipt { name, bytes, mime, sha256: Some(checksum) };
        job.receipt = Some(receipt.clone()); job.state = "stored".into();
        Ok(receipt)
    })
}
pub fn abort(root: &Path, id: &str, lease: &str, reason: &str) -> Result<()> {
    ensure!(["network", "authentication", "source-missing", "storage", "format",
        "video-too-large"].contains(&reason), "invalid-error");
    with_state(root, |state| {
        if state.jobs.get(id).is_some_and(|job| job.state == "stored") { return Ok(()); }
        let job = leased(state, id, lease)?;
        job.state = if reason == "video-too-large" { "unsupported" } else { "failed" }.into();
...
pub fn dispatch(root: &Path, request: &Value) -> Result<Value> {
    ...
    match operation {
        "capture.put" => { put_capture(root, request["envelope"].clone())?; Ok(json!({"queued":true})) }
        "work.pull" => Ok(serde_json::to_value(pull(root)?)?),
        "asset.begin" | "asset.status" => Ok(serde_json::to_value(status(root, text("jobId")?)?)?),
        "asset.append" => {
            let bytes = base64::engine::general_purpose::STANDARD.decode(text("data")?)?;
            let offset = append(root, text("jobId")?, text("lease")?,
                request["offset"].as_u64().context("invalid-offset")?, &bytes)?;
            Ok(json!({"offset":offset}))
        }
        "asset.commit" => Ok(serde_json::to_value(commit(root, text("jobId")?, text("lease")?,
            request["bytes"].as_u64().context("invalid-length")?, text("sha256")?)?)?),
        "asset.abort" => { abort(root, text("jobId")?, text("lease")?, text("reason")?)?; Ok(Value::Null) }
