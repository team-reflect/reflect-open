import { z } from 'zod'
import {
  VIDEO_MAX_BYTES, IMAGE_MAX_BYTES, ASSET_CHUNK_MAX_BYTES,
  archiveJobSchema, isValidUrl, type ArchiveJob,
} from '@reflect/core/x-archive'
import { sendArchiveMessage } from './x-native'
import { getRecords, putRecord, saveChunk, readChunks, removeDownload, type DownloadRecord } from './x-download-store'
...
async function checksum(chunks: Array<{ offset: number; bytes: Uint8Array }>, length: number) {
  ...
  for (const chunk of [...chunks].sort((a, b) => a.offset - b.offset)) {
    if (chunk.offset !== offset || offset + chunk.bytes.length > length) throw new Error('chunk-gap')
...
export async function downloadJob(binding: string, job: ArchiveJob): Promise<void> {
  ...
  let failure: 'network' | 'authentication' | 'source-missing' | 'format' | 'storage' | 'video-too-large' = 'network'
  ...
  try {
    ...
    if (!['video/mp4', 'image/jpeg', 'image/png', 'image/webp', 'image/gif'].includes(mime ?? '')) {
      failure = 'format'; throw new Error(failure)
    }
    ...
    if (!response.body) { failure = 'format'; throw new Error('empty-body') }
    ...
    while (true) {
      ...
      for (let start = 0; start < part.value.byteLength; start += ASSET_CHUNK_MAX_BYTES) {
        ...
        const response = await sendArchiveMessage({
          op: 'asset.append', binding, jobId: job.id, lease: job.lease, offset, data: toBase64(chunk),
        })
        const ack = z.object({ offset: z.number().int() }).parse(response.data)
        if (ack.offset !== record.bytes) throw new Error('offset-conflict')
      }
    }
    if (!record.bytes) { failure = 'format'; throw new Error('empty-body') }
    failure = 'storage'
    ...
    await sendArchiveMessage({
      op: 'asset.abort', binding, jobId: job.id, lease: job.lease, reason: failure,
    }).then(() => removeDownload(id), () => {})
    throw error
  } finally {
...
export async function finishDownload(record: DownloadRecord) {
  ...
  const response = await sendArchiveMessage({
    op: 'asset.status', binding: record.binding, jobId: record.job.id,
  })
  const job = archiveJobSchema.parse(response.data)
  if (job.state === 'stored') { await removeDownload(record.id); return }
  ...
  for (const chunk of (await readChunks(record.id)).sort((a, b) => a.offset - b.offset)) {
    await sendArchiveMessage({
      op: 'asset.append', binding: record.binding, jobId: job.id, lease: record.job.lease,
      offset: chunk.offset, data: toBase64(chunk.bytes),
    })
  }
  await sendArchiveMessage({
    op: 'asset.commit', binding: record.binding, jobId: job.id, lease: record.job.lease,
