...
const wire = likeWireSchema.parse({ envelope: {
  version: 2, kind: 'x-like', id: '00000000-0000-4000-8000-000000000001',
  source: 'extension', postId: '20', capturedAt: '2026-09-09T04:00:00Z',
} })
...
it.each(['invalid-payload', 'unsupported-version'])('retains a like if support changes after the probe: %s', async (code) => {
  send.mockResolvedValueOnce(supported).mockResolvedValueOnce({ ok: false, code, message: 'changed reader' })
...
it('does not probe or change rejection policy for bookmarks', async () => {
  ...
  expect(await sendToHost({ envelope: { ...wire.envelope, kind: 'x-bookmark' } })).toMatchObject({ kind: 'rejected' })
  expect(send).toHaveBeenCalledTimes(1)
})
