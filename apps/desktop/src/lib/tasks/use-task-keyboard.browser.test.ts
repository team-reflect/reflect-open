...
afterEach(() => {
  // vitest-browser-react auto-unmounts each hook, removing its document keydown
  // listener. Without that, a prior test's handler runs first, preventDefaults,
  // and the next bails on it.
  root.remove()
})
...
describe('useTaskKeyboard', () => {
  ...
  it('backs off ⌘⇧K while the inline editor is focused (it handles convert itself)', async () => {
    ...
    const selection = makeSelection({ selected: new Set(['k']), selectedCount: 1 })
    const { onConvertToBullet } = await mount({ selection, tasksByKey: new Map([['k', task()]]) })
  ...
  it('leaves Escape for other handlers when nothing is selected and the query is empty', async () => {
    const { selection } = await mount({ selection: makeSelection({ selectedCount: 0 }), query: '' })
  ...
  it('Return adds a task to today’s daily when nothing is selected', async () => {
    ...
    const { selection } = await mount({ actions: makeActions({ insert }), today: '2026-06-15' })
  ...
  it('backs off entirely while the inline editor is focused', async () => {
    ...
    root.appendChild(editor)
    const selection = makeSelection({ selected: new Set(['k']), selectedCount: 1 })
    const { actions } = await mount({ selection, tasksByKey: new Map([['k', task()]]) })
  ...
  it('in the search box, only Escape acts', async () => {
    const input = document.createElement('input')
    root.appendChild(input)
    const { selection, setQuery } = await mount({ selection: makeSelection({ selectedCount: 1 }) })
