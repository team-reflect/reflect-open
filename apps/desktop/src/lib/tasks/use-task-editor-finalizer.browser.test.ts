...
describe('useTaskEditorFinalizer', () => {
  it('commits a real change, cancels an unchanged one, deletes an emptied one', async () => {
    const changed = await setup('milk')
    changed.type('oat milk')
    changed.api().commit()
    expect(changed.onCommit).toHaveBeenCalledWith('oat milk')

    const unchanged = await setup('milk')
    ...
    const emptied = await setup('milk')
    emptied.type('')
    emptied.api().commit()
    expect(emptied.onDelete).toHaveBeenCalled()
  })

  it('Escape keeps a typed row but removes an empty one; ⌘⌫ always deletes', async () => {
    ...
    const cleared = await setup('milk')
    cleared.type('   ')
    cleared.api().cancel()
    expect(cleared.onDelete).toHaveBeenCalled()

    // ⌘⌫ deletes outright regardless of content.
    const deleted = await setup('milk')
    deleted.type('still here')
    deleted.api().delete()
    expect(deleted.onDelete).toHaveBeenCalled()
  })

  it('Enter (continue) hands the screen the resolved content to persist then insert', async () => {
    const changed = await setup('milk')
    changed.type('oat milk')
  ...
  it('Backspace on an empty row routes to delete-and-select-previous, not plain delete', async () => {
    const h = await setup('')
    h.api().deleteEmpty()
  ...
  it('completes: unchanged toggles, a change saves first, emptied deletes', async () => {
    const unchanged = await setup('milk')
  ...
  it('checkbox toggles: unchanged toggles, a change saves first, emptied deletes', async () => {
    const unchanged = await setup('milk')
  ...
  it('converts to a bullet: unchanged converts as-is, a change saves first, emptied deletes', async () => {
    const unchanged = await setup('milk')
  ...
  it('unmount persists a change via onFlush: never cancels/clears the new selection', async () => {
    const changed = await setup('milk')
    changed.type('oat milk')
  ...
  it('unmount of an unchanged editor does nothing (no cancel, no write)', async () => {
    const unchanged = await setup('milk')
  ...
  it('unmount persists a cleared edit (onFlush with empty content), not drops it', async () => {
    const emptied = await setup('milk')
    emptied.type('   ')
    await emptied.unmount()
    expect(emptied.onFlush).toHaveBeenCalledWith('')
  })

  it('is single-shot: a committed editor does not also flush on unmount', async () => {
    const h = await setup('milk')
    h.type('oat milk')
    h.api().commit()
    await h.unmount()
  ...
  it('unmount flush uses the latest render’s callback, not the mount-time one', async () => {
    const onFlushA = vi.fn()
    const onFlushB = vi.fn()
    const { result, rerender, unmount } = await renderHook(
      ({ onFlush }: { onFlush: (content: string) => void } = { onFlush: onFlushA }) =>
        useTaskEditorFinalizer({
          initial: 'milk',
          onCommit: vi.fn(),
          onContinue: vi.fn(),
    ...
    await rerender({ onFlush: onFlushB }) // e.g. the row re-rendered with a fresh closure
    await unmount()
