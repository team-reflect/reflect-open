...
/**
 * The markdown reference for one PDF annotation: `[<text>](assets/…pdf#page=N)`.
 * The label is the annotated text itself — the reference reads as the quoted
 * passage, not as a file citation; the filename and page stay in the href.
 * An annotation without text falls back to `<filename> - p<N>`. Pasting the
 * link into any note yields a clickable jump back to the PDF's page — the
 * app's `parsePdfHref` already resolves that href shape.
 */
export function annotationReference(assetPath: string, item: AnnotationItem): string {
  ...
  const rawTitle = text === '' ? `${filename} - p${page}` : text
