...
import { isRecordingSupported, useAudioRecorder, type RecorderResult } from './use-audio-recorder'
...
describe('useAudioRecorder', () => {
  ...
  it('discards a sub-half-second recording as a misclick', async () => {
    ...
    let recording: RecorderResult | null = null
    await act(async () => {
  ...
  it('concurrent stops share one in-flight result and stop the recorder once', async () => {
    ...
    let first: RecorderResult | null = null
    let second: RecorderResult | null = null
    await act(async () => {
      const racingStop = result.current.stop()
      const racingStopTwin = result.current.stop()
      ;[first, second] = await Promise.all([racingStop, racingStopTwin])
    })
