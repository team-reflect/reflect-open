...
export interface PendingAudioCapture {
  ...
  mimeType: string
  recordedAt: Date
  /** Placement within the recording session this segment belongs to. */
...
export function useAudioMemoPipeline(
  ...
  const transcriptionPromptRef = useRef(settings.transcriptionPrompt)
  useEffect(() => {
  ...
  const pump = useCallback(async (): Promise<void> => {
    ...
    try {
      while (parkedRef.current === null) {
        ...
        let outcome: Awaited<ReturnType<typeof captureAudioMemoPart>>
        try {
          outcome = await captureAudioMemoPart({
            audio: capture.audio,
