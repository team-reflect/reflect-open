...
describe('SyncConflictNotice', () => {
  it('renders nothing for a note without conflict markers', async () => {
    ...
    // Let the query run, then confirm the notice stayed away.
    await vi.waitFor(() => expect(vi.mocked(getNote)).toHaveBeenCalled())
    await expect.element(page.getByText(/edited on two devices/i)).not.toBeInTheDocument()
  })
  ...
  it('offers mine/theirs/both resolutions for a conflicted note', async () => {
    ...
    await userEvent.click(page.getByRole('button', { name: /keep this device’s version/i }))
    expect(resolution.resolve).toHaveBeenCalledWith('ours')

    await userEvent.click(page.getByRole('button', { name: /keep the other device’s/i }))
    expect(resolution.resolve).toHaveBeenCalledWith('theirs')

    await userEvent.click(page.getByRole('button', { name: /keep both/i }))
