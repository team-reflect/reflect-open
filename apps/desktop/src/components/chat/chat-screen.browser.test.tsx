...
// Radix Select scrolls the selected option into view when the listbox opens;
// guarded so the real browser implementation wins when present.
Element.prototype.scrollIntoView ??= () => {}
// shadcn's MessageScroller drives the viewport with scrollTo; guarded the same way.
...
/** Native file drop: build a DataTransfer and dispatch a cancelable drop. */
...
describe('ChatScreen', () => {
  it('shows the add-a-provider call to action when nothing is configured', async () => {
    ...
    await expect.element(view.getByRole('button', { name: /add an ai provider/i })).toBeInTheDocument()
    await expect.element(view.getByLabelText('Chat message')).not.toBeInTheDocument()
  })
  ...
  it('runs a grounded turn: user bubble, search chip, cited answer', async () => {
    ...
    await expect.element(view.getByText('when does atlas ship?', { exact: true })).toBeInTheDocument()
    ...
    await userEvent.click(view.getByRole('button', { name: 'Atlas', exact: true }))
  ...
  it('opens cited wiki links from settled chat markdown', async () => {
    ...
    await userEvent.click(view.getByRole('button', { name: 'Open Atlas' }))
  ...
  it('offers the provider catalog in the picker, keeping a custom model selectable', async () => {
    ...
    await userEvent.click(view.getByRole('combobox', { name: 'Model' }))

    await expect.element(page.getByText('OpenAI', { exact: true })).toBeInTheDocument()
    const labels = page.getByRole('option').elements().map((option) => option.textContent)
  ...
  it('routes the turn to the picked catalog model', async () => {
    ...
    await userEvent.click(view.getByRole('combobox', { name: 'Model' }))
    await userEvent.click(page.getByRole('option', { name: 'GPT-5.5' }))
  ...
  it('starts the picker on the model persisted from the last session', async () => {
    ...
    const picked = page.getByRole('option', { name: 'GPT-5.5' })
  ...
  it('still sends the turn, without an overview, when the context load fails', async () => {
    ...
    await expect.element(view.getByText('Hi.', { exact: true })).toBeInTheDocument()
  })
  ...
  it('renders streaming text as plain text until the turn settles', async () => {
    ...
    await expect.element(view.getByText('Streaming **markdown**', { exact: true })).toBeInTheDocument()
    await expect.element(view.getByTestId('markdown-preview')).not.toBeInTheDocument()
  })
  ...
  it('rejects a second send fired before the first one has rendered', async () => {
    ...
    await expect.element(view.getByText('two', { exact: true })).not.toBeInTheDocument()
  })
  ...
  it('sends a dropped photo with no text as an image-only message', async () => {
    ...
    await expect.element(view.getByRole('button', { name: 'Remove cat.png' })).not.toBeInTheDocument()
  ...
  it('a drop still reading when New chat clears the session never lands', async () => {
    ...
    await expect.element(view.getByAltText('cat.png')).not.toBeInTheDocument()
  })
  ...
  it('claims non-image file drops so the webview never navigates to them', async () => {
    ...
    await expect.element(view.getByAltText('notes.txt')).not.toBeInTheDocument()
  })
  ...
  it('a removed attachment never sends', async () => {
    ...
    dropFiles(view.getByLabelText('Chat message').element(), [pngFile('cat.png')])
    await userEvent.click(view.getByRole('button', { name: 'Remove cat.png' }))
    await expect.element(view.getByAltText('cat.png')).not.toBeInTheDocument()
  ...
  it('New chat clears the conversation', async () => {
    ...
    await expect.element(view.getByTestId('markdown-preview')).not.toBeInTheDocument()
    await expect.element(view.getByText('hey', { exact: true })).not.toBeInTheDocument()
  })
})
