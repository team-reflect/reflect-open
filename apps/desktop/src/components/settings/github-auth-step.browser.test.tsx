...
async function switchToPat(): Promise<void> {
  await userEvent.click(
    page.getByRole('button', { name: /use a personal access token instead/i }),
  )
  await expect.element(page.getByLabelText('Personal access token', { exact: true })).toBeInTheDocument()
}
...
async function renderCodeView(): Promise<void> {
  ...
  await userEvent.click(page.getByRole('button', { name: 'Sign in with GitHub', exact: true }))
  await expect.element(page.getByText('ABCD-1234', { exact: true })).toBeInTheDocument()
}
...
describe('GithubAuthStep', () => {
  ...
  it('stores a pasted PAT, verifies it against GitHub, and reports the identity', async () => {
    ...
    await userEvent.fill(page.getByLabelText('Personal access token', { exact: true }), '  github_pat_abc  ')
    await userEvent.click(page.getByRole('button', { name: 'Save token', exact: true }))
  ...
  it('fails a rejected token at entry and clears it from the keychain', async () => {
    ...
    await userEvent.fill(page.getByLabelText('Personal access token', { exact: true }), 'github_pat_typo')
    await userEvent.click(page.getByRole('button', { name: 'Save token', exact: true }))

    await expect.element(page.getByText(/rejected the token/i)).toBeInTheDocument()
  ...
  it('rejects an empty token with an inline message', async () => {
    ...
    await expect.element(page.getByText('Paste a token first.', { exact: true })).toBeInTheDocument()
    expect(onAuthed).not.toHaveBeenCalled()
  })
  ...
  it('names the backup repository in the token instructions when known', async () => {
    ...
    await expect.element(page.getByText('my-notes-backup', { exact: true })).toBeInTheDocument()
  })
  ...
  it('reports auth exactly once when the mount probe races a fresh sign-in', async () => {
    ...
    await userEvent.fill(page.getByLabelText('Personal access token', { exact: true }), 'github_pat_new')
    await userEvent.click(page.getByRole('button', { name: 'Save token', exact: true }))
  ...
  it('copies the code to the clipboard before opening GitHub', async () => {
    ...
    await userEvent.click(page.getByRole('button', { name: 'Copy code and open GitHub', exact: true }))
    ...
    await expect.element(page.getByText(/code copied/i)).toBeInTheDocument()
  })
  ...
  it('holds the GitHub handoff when the clipboard is unavailable', async () => {
    ...
    await expect.element(page.getByText(/select the code above/i)).toBeInTheDocument()
    expect(openedUrls).not.toHaveBeenCalled()

    await userEvent.click(page.getByRole('button', { name: 'Open GitHub', exact: true }))
    await vi.waitFor(() =>
      expect(openedUrls).toHaveBeenCalledWith('https://github.com/login/device'),
    )
  })
