...
describe('AudioMemoProvider', () => {
  it('toggle records, then stops and hands the recording to the capture action', async () => {
    const { result } = await renderHook(() => useAudioMemo(), { wrapper })
  ...
  it('mounts one reconciler per graph session, started immediately, disposed on unmount', async () => {
    const { unmount } = await renderHook(() => useAudioMemo(), { wrapper })
  ...
  it('collapsing the sidebar mid-recording stops and saves', async () => {
    const { result, rerender } = await renderHook(() => useAudioMemo(), { wrapper })
  ...
  it('a denied microphone maps to platform-appropriate guidance, with no retry', async () => {
    ...
    // The copy is platform-specific: macOS names the System Settings path, other
    // platforms give generic guidance. Assert the branch for the host browser.
