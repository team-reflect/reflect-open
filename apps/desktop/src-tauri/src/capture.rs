...
const CAPTURE_MEDIA_MAX_BYTES: usize = 10 * 1024 * 1024;

/// Fetch bounded HTTPS JSON without redirects for capture enrichment.
#[tauri::command]
...
/// Fetch a public raster image and return a bounded JPEG for the privacy-gated writer.
#[tauri::command]
pub async fn capture_media_fetch(url: String) -> AppResult<String> {
    ...
    .await
    .map_err(|err| AppError::io(format!("capture image task failed: {err}")))??;
