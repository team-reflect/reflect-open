...
export function chatSystemPrompt({
  ...
  return [
    ...
    '- Reflect-managed images and PDFs live under the graph-root assets/ tree. Nested notes may link them as ../assets/sketch.png; normalize that to assets/sketch.png when you pass asset paths to read_assets. The link alone tells you nothing about the file — read_assets returns its stored description and text transcription. Search also matches attachment text, so a matching note may mention your query only inside an attachment.',
