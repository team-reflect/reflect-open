...
/**
 * A syntax range to drop from plain text. `text`, when set, is emitted in its
 * place: a link's destination is syntax, but keeping it in the searchable text
 * is what lets a note be found by the URL it links to.
 */
...
export function buildPlainText(body: string, cuts: Cut[], literalRanges: Span[]): string {
